...
try:
    ...
    from utils import Archive, FormatUtils
except ImportError:
    from .respot import Respot, RespotUtils
    from .tagger import AudioTagger
    from .utils import Archive, FormatUtils
...
class ZSpotify:
    ...
    def parse_args(self):
        ...
        parser.add_argument('--limit', help='Search limit', default=_LIMIT_RESULTS, type=int)
        parser.add_argument(
            '-f',
            '--force-premium',
    ...
    def download_track(self, track_id, path=None, caller=None):
        ...
        print(
            f'Downloading {filename} with format: {FormatUtils.RED}{self.args.audio_format}{FormatUtils.RESET}.'
        )
        output_path = self.respot.download(track_id, temp_path, self.args.audio_format, True)
    ...
    def start(self):
        ...
        if self.args.all_playlists:
            ...
            with open(self.args.bulk_download) as file:
                for line in file:
