...
class Respot:
    def __init__(self, config_dir, force_premium, credentials, audio_format, antiban_wait_time):
...
class RespotAuth:
    ...
    def refresh_token(self) -> tuple[str, str]:
        self.session = (
            Session.Builder().stored_file(stored_credentials=str(self.credentials)).create()
        )
...
class RespotRequest:
    ...
    def get_track_info(self, track_id) -> dict:
        ...
        try:
            ...
            lyrics = get_lyrics(track_id, self.auth.session)
    ...
    def get_album_info(self, album_id):
        """Returns album name"""
        resp = self.authorized_get_request(f'https://api.spotify.com/v1/albums/{album_id}').json()
    ...
    def get_artist_info(self, artist_id):
        ...
        try:
            info = json.loads(
                self.authorized_get_request('https://api.spotify.com/v1/artists/' + artist_id).text
            )
    ...
    def get_show_info(self, show_id):
        """returns show info"""
        resp = self.authorized_get_request(f'https://api.spotify.com/v1/shows/{show_id}').json()
        return {
...
class RespotUtils:
    ...
    def parse_url(search_input) -> dict:
        ...
        track_uri_search = re.search(r'^spotify:track:(?P<TrackID>[0-9a-zA-Z]{22})$', search_input)
        track_url_search = re.search(
            r'^(https?://)?open\.spotify\.com/track/(?P<TrackID>[0-9a-zA-Z]{22})(\?si=.+?)?$',
            search_input,
        )

        album_uri_search = re.search(r'^spotify:album:(?P<AlbumID>[0-9a-zA-Z]{22})$', search_input)
        ...
        show_uri_search = re.search(r'^spotify:show:(?P<ShowID>[0-9a-zA-Z]{22})$', search_input)
        ...
        if playlist_uri_search is not None or playlist_url_search is not None:
            playlist_id_str = (
                playlist_uri_search if playlist_uri_search is not None else playlist_url_search
            ).group('PlaylistID')
        else:
